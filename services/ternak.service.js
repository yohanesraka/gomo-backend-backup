...
class _ternak {
    ...
    getTernak = async (req) => {
        try {
            ...
            (req.query.status_keluar = null), (req.query.tanggal_keluar = null);
            // Query data
            const list = await this.db.Ternak.findAll({
                attributes: ["id_ternak", "image", "jenis_kelamin", "id_dam", "id_sire", "tanggal_lahir", "tanggal_masuk", "status_perah", "tanggal_keluar", "status_keluar", "createdAt", "updatedAt"],
                include: [
                    {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if (list[i].dataValues.fase && list[i].dataValues.fase.dataValues.fase === "Laktasi") {
                    const lactationAge =
                        list[i].dataValues.riwayat_fase.length > 0 ? Math.round((new Date() - new Date(list[i].dataValues.riwayat_fase[list[i].dataValues.riwayat_fase.length - 1].dataValues.tanggal)) / (1000 * 60 * 60 * 24)) : 0;
                ...
                list[i].dataValues.kebutuhan_pakan = (
                    (list[i].dataValues.timbangan.length > 0 ? list[i].dataValues.timbangan[list[i].dataValues.timbangan.length - 1].dataValues.berat : 0) *
                    ((list[i].dataValues.kandang && list[i].dataValues.kandang.persentase_kebutuhan_pakan ? list[i].dataValues.kandang.persentase_kebutuhan_pakan : 0) / 100)
                ).toFixed(2);
    ...
    getDataIndukan = async (req) => {
        try {
            ...
            // Query Data
            const list = await this.db.Ternak.findAll({
                attributes: ["id_ternak", "rf_id", "image", "jenis_kelamin", "id_dam", "id_sire", "tanggal_lahir", "tanggal_masuk", "tanggal_keluar", "status_keluar", "createdAt", "updatedAt"],
                include: [
                    {
                        model: this.db.Bangsa,
                        as: "bangsa",
