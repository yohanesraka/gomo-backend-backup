...
class _produksiSusu {
  ...
  getTotalDataProduksi = async (req) => {
    try {
      ...
      const uniqueDates = Array.from(new Set(ProduksiSusu.map((row) => row.tanggal_produksi.toLocaleDateString("id-ID", {
        timeZone: "Asia/Jakarta"
      }))));

      uniqueDates.forEach((date, index) => {
        const ProduksiHarian = ProduksiSusu.filter((row) => row.tanggal_produksi.toLocaleDateString("id-ID", {
          timeZone: "Asia/Jakarta"
        }) === date);
        // produksi pagi
        ...
        console.log(produksiPagi);
        console.log(produksiSore);
      });
